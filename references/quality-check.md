...
- [ ] **行间距 ≥ 1.5** - 相邻文本框y坐标差至少1.5单位
- [ ] **分组间距 ≥ 2** - 题目/答案/分析之间至少2单位
- [ ] **图形间距 ≥ 1** - 文本与最近图形至少1单位
- [ ] **边界检查** - 所有文本在可视坐标范围内
...
**解决：** 增大文本框y坐标间距，确保 ≥ 1.5单位
```xml
<!-- 正确：间距1.5 -->
<command name="Text"><input a1="(-10, 8)"/></command>  <!-- 第一行 -->
<command name="Text"><input a1="(-10, 6.5)"/></command> <!-- 第二行，差1.5 -->

<!-- 错误：间距只有0.5，容易重叠 -->
<command name="Text"><input a1="(-10, 8)"/></command>  <!-- 第一行 -->
<command name="Text"><input a1="(-10, 7.5)"/></command> <!-- 第二行，差0.5 ❌ -->
```
...
<!-- 原图菱形：AC水平，BD竖直 -->
<!-- 正确：保持AC水平 -->
A(-6.928, 0)  C(6.928, 0)    ← AC水平 ✓
B(0, -4)      D(0, 4)        ← BD竖直 ✓

<!-- 错误：旋转90° -->
A(0, 6.928)   C(0, -6.928)   ← AC竖直 ❌
B(-4, 0)      D(4, 0)        ← BD水平 ❌
```
